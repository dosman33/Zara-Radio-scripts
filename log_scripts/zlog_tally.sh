#!/bin/bash
# 03-01-12 / dosman / sort zara log files and give file counts
# 03-05-12 / dosman / added ftpdownload function, grabs complete previous days zara log (lastzarasongs usually misses last song)
###################################################################################
...
OUT=${OUTPATH}/current_month_playout_tally.txt
LOGDIR=${OUTPATH}/playout
...
ftpdownload() {
FILE=$1
FTP="/usr/bin/ftp -i -n -u"
HOST=0.0.0.0
FTPUSER=anonymous
FTPPASSWD=changeme
DIR=/changeme
#UPLOADDIR=/station/updated_reports
...
if [[ $1 == "-h" || $1 == "--h" || $1 == "-?" || $1 == "--?" ]];then
	echo "Usage:"
	echo "$0     - process Zara playout log and tally top songs for the current month"
	echo "$0 -p  - process Zara playout log and tally top songs for the previous month"
	exit
elif [[ $1 == "-p" ]];then
	# process previous month
	myDATE=`date_mangle 1 `
	myDATE2=`echo $myDATE | tr " " "-"`
	ftpdownload ${myDATE2}.log
	YM=`echo $myDATE | awk '{print $1"-"$2}'`
	MONTH=`echo $myDATE | awk '{print $2}'`
	YEAR=`echo $myDATE | awk '{print $1}'`

	OUT=${OUTPATH}/20${YEAR}-${MONTH}_playout_tally.txt
	find $LOGDIR -type f -name "20${YEAR}-${MONTH}*.log" -exec cat {} > $TMP1 \;
	echo "$STATION track tally for 20${YM}" > $OUT
	echo "------------------------------------------------------------------------------------" >> $OUT
	egrep -v "      warning|        error|shutdown|startup|Maximum wait reached|${METACOMMAND}|updated_reports|jingles|playlists|^[a-z]|^[A-Z]|^--|^==|^$" $TMP1 | cut -c 16- | sort | uniq -c | sort -rn >> $OUT
	...
	#echo "<html><title>$STATION top songs for $MONTH $YEAR</title><body><pre>" > $OUT
	echo "$STATION track tally for ${YM}" > $OUT
	echo "------------------------------------------------------------------------------------" >> $OUT
	egrep -v "      warning|        error|shutdown|startup|Maximum wait reached|${METACOMMAND}|updated_reports|jingles|playlists|^[a-z]|^[A-Z]|^--|^==|^$" $TMP1 | cut -c 16- | sort | uniq -c | sort -rn >> $OUT
fi
cleanup
